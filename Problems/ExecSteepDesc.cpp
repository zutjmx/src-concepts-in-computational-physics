//  Execute the CSteepDesc class, Appendix H: Deterministic Optimization
...
//   double dXstart=1.9,
//      dYstart=0.4;
   CSteepDesc S(f,GradFx,GradFy);  //  work with Eq. H.6
//   CSteepDesc S(f1,GradFx1,GradFy1);  //  work with Eq. H.51
   string sListFile("D:\\CompPhys\\Steep1.dat");
        //  data file for Fig. H.1, first path
//   string sListFile("D:\\CompPhys\\Steep2.dat");
        //  data file for Fig. H.1, second path
//   string sListFile("D:\\CompPhys\\Steep3.dat");
        //  data file for Fig. H.1, third path
//   string sListFile("D:\\CompPhys\\Steep4.dat");
        //  data file for Fig. H.3

//   Appendix H, steepest descent

   if (ret = S.Iterate(dXstart,dYstart)) {
      if (ret == 1)
         cout << "Error1 in bisection routine" << endl;
      else if (ret == 2)
         cout << "Error2 no convergence in bisection routine" << endl;
      return ret;
   }
   ...
   for (register int i=0; i<S.m_nImax; ++i)
      ofLst << setw(12) << S.m_vdXiter[i] << setw(12) << S.m_vdYiter[i] << endl;
   ofLst.close();
   return ret;
}
